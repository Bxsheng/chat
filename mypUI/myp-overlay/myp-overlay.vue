<template>
	<view :class="['myp-overlay', 'myp-bg-'+bgType]"
		  @touchmove.prevent
		  v-if="helpShow"  ref="myp-overlay" @click.stop="overlayClose" :style="overlayStyle + noWeexTransition">
		<slot></slot>
	</view>
</template>
		...
		methods: {
			...
			weexAppearOverlay(bool, duration = this.duration, fromOverlay = false) {
				...
				if (fromOverlay) {
					...
					this.$emit('overlayClicking', {})
					if (hasAnimation) {
						const overlayEl = this.$refs['myp-overlay'];
